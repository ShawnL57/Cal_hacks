...
class AnnoyingDuck {
    private duckElements: HTMLDivElement[] = []; // Track multiple ducks
    ...
    private createDuck(): void {
        ...
        duckElement.addEventListener('animationend', () => {
            ...
            if (index > -1) this.duckElements.splice(index, 1);
        });
    ...
    private updateFocusWindow(focusScore: number): void {
        const now = Date.now();
    ...
    private checkSustainedFocusDrop(): void {
        if (this.focusWindow.length === 0) return;
        ...
        if (avgFocus < this.FOCUS_THRESHOLD) {
            ...
            if (now - this.lastFocusDropLoggedAt < this.LOG_DEBOUNCE_MS) return;
    ...
    private getScrollPercent(): number | null {
        try {
    ...
    private logFocusDrop(scrollPercent: number): void {
